...
A state-of-the-art, hardware-accelerated Command & Control (C2) operator console designed for the **AERIS-10 Phased-Array Radar**. Engineered with an elite tactical dark mode aesthetic, hardware-accelerated 3D WebGL holodome tracking, discrete 2D sweep persistence plotting, fuzzy-search command palette, and zero-overhead performance profiles.
...
## 📡 Hardware Project Accreditation & Credits
This software suite is built as a custom tactical operator console designed to interface with the open-source hardware project **AERIS-10 Phased-Array LFM Radar**:

* **Original Hardware Designer & Creator:** Nawfal Motii
* **Official Hardware Source Code & Gerber Layouts:** [NawfalMotii79/PLFM_RADAR](https://github.com/NawfalMotii79/PLFM_RADAR)
* **Project Specifications:** Features X-Band 10.5 GHz radar frequencies with active electronic beam steering (AERIS-10N Nexus & AERIS-10E Extended).
...
* **Zero-Jitter Tactical Odometers:** Mono-width mechanical digit tumbler strips translating Uptime and Frame counters smoothly without distracting text jitters.
...
* [Radar3D.tsx](frontend/src/components/radar/Radar3D.tsx) — WebGL 3D Holographic dome and raycaster.
* [PPICanvas.tsx](frontend/src/components/radar/PPICanvas.tsx) — Main 2D PPI Canvas viewport.
* [ppiWorker.ts](frontend/src/workers/ppiWorker.ts) — Multi-threaded 2D sweep and discrete trail engine.
* [TopBar.tsx](frontend/src/components/layout/TopBar.tsx) — Primary Header, command palette launcher, and pinned lost cards.
...
To successfully launch the console with active radar telemetry streams, you must boot both the telemetry backend and the frontend interface.

#### 1. Start the Telemetry Backend Server
From the root directory of the project, start the lightweight Node.js telemetry simulation server:
```bash
node mock_server.js
```
This boots the local high-frequency WebSocket feed on `ws://127.0.0.1:8765/ws/radar`.

#### 2. Start the React Frontend Console
Open a new terminal window, navigate into the frontend directory, install dependencies, and launch the development server:
```bash
cd frontend
npm install
npm run dev
